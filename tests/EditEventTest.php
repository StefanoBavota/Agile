...
class EditEventTest extends TestCase
{
    ...
    public function test_editEvent()
    {
        ...
        $event = ['img' => 'image', 'name' => 'test', 'description' => 'test2', 'data' => '2021-05-24', 'posti' => 0, 'user_id' => 1, 'music_type_id' => 1];
        $eventMgr->createEvent($event['img'], $event['name'], $event['description'], $event['data'], $event['posti'], $event['user_id'], $event['music_type_id']);

        $eventEdited = ['img' => 'imageEdited', 'name' => 'testEdited', 'description' => 'test2Edited', 'data' => '2021-05-24', 'posti' => '66', 'user_id' => '1', 'music_type_id' => '1'];
        $eventMgr->editEvent(1, $eventEdited['img'], $eventEdited['name'], $eventEdited['description'], $eventEdited['data'], $eventEdited['posti'], $eventEdited['music_type_id']);
        $dbEventEdited = $eventMgr->getEventByIdTest(1)[0];
        
        unset($dbEventEdited['id']);
        $this->assertEquals($eventEdited, $dbEventEdited);
    }
}
